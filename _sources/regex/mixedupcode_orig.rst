...
.. parsonsprob:: re_mixed4
    ...
    Create a function called ``test_match`` that takes in two string parameters, ``pattern`` and ``sequence``, and returns ``"Match!"`` if 
    ``pattern`` and ``sequence`` are the same and ``"Not a match!"`` otherwise. For example, ``test_match("Puppy", "Puppies")`` should return 
    ``"Not a match!"``.
    -----
    import re
    =====
    import regex #paired
    =====
    def test_match(pattern, sequence):
    =====
...
.. activecode:: re_mixed4_ac
    ...
    Write a function called ``test_match`` that takes in two string parameters, ``pattern`` and ``sequence``, and returns ``"Match!"`` if 
    ``pattern`` and ``sequence`` are the same and ``"Not a match!"`` otherwise. For example, ``test_match("Puppy", "Puppies")`` should return 
    ``"Not a match!"``.
    ~~~~
    import re
...
.. parsonsprob:: re_mixed6
    ...
    =====
        if re.findall('\$\d+(?:\.\d{2})?', string):
    =====
            return re.findall('\$\d+(?:\.\d{2})?', string)[0]
    =====
            return re.findall('\$\d*(?:\.\d{2})?', string)[0] #paired
    =====
            return re.findall('\$\d*(?:\.\d{2})?', string)[0] #paired
    =====
        else:
...
.. parsonsprob:: re_mixed7
    ...
    =====
        return re.findall('\S*@\S*\.\w*', string) #paired
...
.. parsonsprob:: re_mixed9
    ...
    =====
        for item in lst:
    =====
            if re.findall('\w+@(\w+\.\w+)', item):
    =====
                new_lst.append(re.findall('\w+@(\w+\.\w+)', item)[0])
    =====
                new_lst.append(re.findall('\w+@(\w+\.\w+)', item)) #paired
    =====
                new_lst.append(item) #paired
    =====
        return new_lst
...
.. parsonsprob:: re_mixed10
    ...
    =====
        if re.search(r"[^AEIOU]+[AEIOU]{2,4}[^AEIOU]+", string):
    =====
        if re.search(r".+[AEIOU]{2,4}[^AEIOU]+$", string): #paired
    =====
            return string
    =====
        else:
