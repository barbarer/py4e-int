...
.. parsonsprob:: mixedupcode_bs4_umich
  ...
  Use Beautiful Soup to find all of the ``a`` (link) tags in the div tag with the ``column`` class and ``news-items`` id. Then, print the link associated with each ``a`` tag or ``None`` if there isn't one.
  -----
  from bs4 import BeautifulSoup
  import requests
  =====
...
.. parsonsprob:: mixedupcode_bs4_courses
    ...
    We want to print a description of three SI courses. Get a soup from each URL with an ending from ``endings``, get the first ``div`` tag with ``class = 'body wysiwyg-content'``, and then extract the text from the first paragraph
    -----
    from bs4 import BeautifulSoup
    import requests
    =====
